...
export default function Sidebar({
  ...
  const togglefoldingSidebar = () => {
    setfoldingState((foldingState : any) => !foldingState);
  }
  ...
  const onMouseMove = (e: React.MouseEvent) => {
    if (dragging && sideBarWidth && separatorXPosition) {
      const newSideBarWidth = sideBarWidth + e.clientX - separatorXPosition;
      console.log("x => " + newSideBarWidth);
      setSeparatorXPosition(e.clientX);

      // 最小幅
      if (newSideBarWidth < 50) {
        setSideBarWidth(50);
        return;
      }
      // 最大幅
      if (wrapperWidth) {
        if (newSideBarWidth > wrapperWidth - 200) {
          setSideBarWidth(wrapperWidth - 200);
          return;
        }
      }
      setSideBarWidth(newSideBarWidth);
      localStorage.setItem('sidebarWidth', JSON.stringify(newSideBarWidth))
    }
  };
  ...
  useEffect(() => {
    console.log("b => " + sideBarWidth);
    console.log("c => " + wrapperWidth);
  ...
  return (
    <div className="sidebar" ref={sideBarRef}>
      <div className="sidebar__head">
        <h2>SIDE : {sideBarWidth} x MAIN : {wrapperWidth - sideBarWidth}</h2>
        <div className="side-folding-button">
          <IconButton size="small" color="primary" onClick={togglefoldingSidebar}>
            { foldingState ? <WestIcon fontSize="small" /> : <EastIcon fontSize="small" /> }
          </IconButton>
        </div>
      </div>
      <div className="sidebar__scroll-inner">
        <p>
      ...
      <div className="side-dragging-aria">
        <div 
